...
async def casar(ctx, user: discord.User=None):
	if user is None:
		...
		try:
			...
			rpg = tutorial["rpg"]
			rpg = tutorial.rpg.find_one({"_id":str(ctx.author.id)})
			rpgg = tutorial.rpg.find_one({"_id":str(user.id)})
			if rpg is None:
				await ctx.send(f"<:erro:581791491467378699> | {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
			if rpgg is None:
				...
				await ctx.send(f"💍 | {ctx.author.mention} e {user.mention} Agora Vocês Estão Casados! Felicidades Aos 2")
				if emoji == '❌':
					await msg.delete()
					await ctx.send('{user.mention} Não aceitou o pedido de casamento de {ctx.author.mention}.')
       ...
       try:
          ...
          if rpg is None:
             await ctx.send(f"🎲 {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
              ...
              moedas2 = int(tutorial.rpg.find_one({"_id": ctx.author.id}))
              tutorial.rpg.update_one({"_id":str(ctx.author.id)}, {"$set":{"coins":moedas2 - coins}})
