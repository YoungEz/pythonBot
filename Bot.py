...
@bot.command(pass_context=True)
async def rgservidor(ctx):
		tutorial = mongo["eventos"]
		rpg = tutorial["rpg"]
		rpg = tutorial.rpg.find_one({"_id":str(ctx.guild.id)})
		if rpg is None:
			servidor = {"_id":str(ctx.guild.id),"usuario":str(ctx.guild.name), "canalwel_id":None,"canallogs_id":None, "canalbye_id":None,"welcome":False,"adeus":False,"logs":False, "a":False, "b":False}
			tutorial.rpg.insert_one(servidor).inserted_id
			await ctx.send(f"<:Registro:581790276163600424> | {ctx.author.mention}, seu servidor foi registrado no banco de dados com sucesso!")
		else:
			await ctx.send(f"<:erro:581791491467378699> | {ctx.author.mention}, Seu servidor já está registrado no banco de dados!")
...
async def config(ctx):
	if ctx.invoked_subcommand is None:
		try:
			...
			rpg = tutorial.rpg.find_one({"_id":str(ctx.guild.id)})
			if rpg is None:
				await ctx.send(f'{ctx.author.mention} Seu servidor ainda não esta registrado em meu sistema, use `s!rgservidor`')
			else:
				welcome = rpg["welcome"]
				adeus = rpg["adeus"]
				logs = rpg["logs"]
				canallogs = rpg["canallogs_id"]
				canalwel = rpg["canalwel_id"]
				canalbye = rpg["canalbye_id"]
			
				if welcome is False:
					emoji = "<:desativado:582992474306379776>"
				else:
					emoji = "<:Ativado:582992389061476372>"
				if adeus is False:
					emojii = "<:desativado:582992474306379776>"
				else:
					emojii = "<:Ativado:582992389061476372>"	
				if logs is False:
					emooji = "<:desativado:582992474306379776>"
				else:
					emoji = "<:Ativado:582992389061476372>"
				if canalwel is None:
					canal = "Canal não definido use `s!config bemvindo #canal` para definir"
				else:
				
					cn = bot.get_channel(canalwel)
					canal = "ctx.can.mention"
				
					
				if canalbye is None:
					cana = "Canal não definido use `s!config adeus #canal` para definir"
				else:
				
					can = bot.get_channel(canalbye)
					cana = "ctx.can.mention"
					
			
				if canallogs is None:
					canaal = "Canal não definido use `s!config logs #canal` para definir"
				else:
				
					canaal = bot.get_channel(canallogs)
														
					emooji = "<:Ativado:582992389061476372>"
				p = discord.Embed(title='Painel De Configuração', color=0xff00ab)
			p.add_field(name="**Bem-Vindo**", value=f"**Status:** {emoji}\n **Canal Definido: {canal}**")
			p.add_field(name="**Adeus**", value=f"**Status:** {emojii}\n**Canal Definido: {cana}**")
			p.add_field(name="**Logs**", value=f"**Status:** {emooji}\n**Canal Definido: {canaal}**")
			p.set_footer(text="Painel De Configuração Shiryu")
			await ctx.send(embed=p)
		except Exception as e:
			await ctx.send(f"[Erro] {e}")
   	
		 		 		 		 		 		 		
@config.command(pass_context=True, aliases=['welcome'])
async def bemvindo(ctx, canal: discord.TextChannel=None):
	if canal is None:
		...
		try:
			...
			rpg = tutorial.rpg.find_one({"_id":str(ctx.guild.id)})
			if rpg is None:
				await ctx.send(f"Olá {ctx.author.mention}, seu servidor não esta no meu sistema, use `s!rgservidor` para adicionar seu servidor")
			else:
				
				tutorial.rpg.update_one({"_id":str(ctx.guild.id)}, {"$set":{"canalwel_id":int(canal.id)}})
				await ctx.send(f"{ctx.author.mention}, Canal de Bem Vindo Definido em {canal.mention}")
		except Exception as e:
				await ctx.send(f"[Erro] {e}") 	 		 		 		 		 		 		 		
