...
async def learderboard(ctx):
	try:
		...
		tutorial = mongo["tutorial"]
		rpg = tutorial["rpg"]
		sort = tutorial.rpg.find().limit(10).sort('coins', -1)
		users = []
		rank = ""
		for d in sort:
			users.append(f'{tutorial["rpg"]} - `{rpg["coins"]}`')
...
@bot.command(pass_context=True, aliases=['marry'])
async def casar(ctx, user: discord.User=None):
	if user == user.bot:
		await ctx.send(f'{ctx.author.mention} Você não pode se casar com um bot')
	if user is None:
		await ctx.send(f'{ctx.author.mention}, Mencione Alguem Para Se Casar')
	else:
		try:
			mongo = MongoClient(url)
			tutorial = mongo["tutorial"]
			rpg = tutorial["rpg"]
			rpg = tutorial.rpg.find_one({"_id":str(ctx.author.id)})
			rp = tutorial.rpg.find_one({"_id":str(user.id)})
			if rpg is None:
				await ctx.send(f"<:erro:581791491467378699> | {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
			if rp is None:
				await ctx.send(f"<:erro:581791491467378699> | {ctx.author.mention}, O usuário mencionado não está registrado no sistema, use s!registro para se registrar!")				
			else:
				msg = await ctx.send(f'{user.mention} Deseja se caser com {ctx.author.mention}? reaja com 💍 sim - ❌ não')
				...
				if emoji == '💍':
					await msg.delete()
				frase = f"Casado(a) Com {user}"
				frase2 = f"Casado(a) Com {ctx.author}"
				tutorial.rpg.update_one({"_id":str(ctx.author.id)}, {"$set":{"ship":str(frase)}})
				tutorial.rpg.update_one({"_id":str(user.id)}, {"$set":{"ship":str(frase2)}})				
				msg = await ctx.send(f"💍 | {ctx.author.mention} e {user.mention} Agora Vocês Estão Casados! Felicidades Aos 2")
				if emoji == '❌':
					await msg.delete()
					msg = await ctx.send('{user.mention} Não aceitou o pedido de casamento de {ctx.author.mention}.')
          ...
          if rpg is None:
             await ctx.send(f"🎲 {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
          	...
          	if flip == 1:
          		...
          		vg = moedas + aposta
          		await ctx.send(f"<a:money:581670005306032130> | {ctx.author.mention}, Ganhou a aposta e recebeu `{vg}` Ryucoins.")
          	if flip == 2:
          		moedas = int(rpg["coins"]) - int(aposta)
          		tutorial.rpg.update_one({"_id":str(ctx.author.id)}, {"$set":{"coins":int(moedas) - aposta}})
          		vg = moedas - aposta
          		await ctx.send(f"<a:money:581670005306032130> | {ctx.author.mention}, Perdeu a aposta e perdeu `{vg}` Ryucoins.")
          		if moedas < aposta:
