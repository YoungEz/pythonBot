		...
		try:
			...
			if rpg is None:
				...
				if logs is False:
					...
					emoji = "<:Ativado:582992389061476372>"
				if canalwel == 0:
					canal = "Canal dão definido use `s!config bemvindo #canal` para definir"
				else:
				
					canal = bot.get_channel(canalwel)
				if canalbye == 0:
					cana = "Canal dão definido use `s!config adeus #canal` para definir"
				else:
				
					cana = bot.get_channel(canalbye)
				if canallogs == 0:
					canaal = "Canal dão definido use `s!config logs #canal` para definir"
				else:
