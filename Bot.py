	...
	if user == ctx.author:
		...
		try:
			...
			if rpg is None:
				...
				await ctx.send(f"🥈 {ctx.author.mention}, você deu `1` reputações para {user.mention} Agora ele tem um total de `{reput}` reputações.")
		except Exception as e:
			await ctx.send(f"[Erro] {e}")
@rep.error
       ...
       try:
          ...
          if rpg is None:
             await ctx.send(f"🎲 {ctx.author.mention}, você não está registrado no sistema, use s!registro para se registrar!")
              ...
              await ctx.send(f"💸 {ctx.author.mention}, transferiu {coins} ryucoins com sucesso para {member.mention}.")
              if moedas - coins < 0:
