...
async def help(ctx):
	...
	try:
		while True:
			...
			if emoji == 'ℹ':
				await msg.delete()
				embed_help = discord.Embed(description="📙┃Menu de ajuda Informações")
			...
			if emoji == '🚀':
				await msg.delete()
				embed_help = discord.Embed(description="📙┃Menu de ajuda")
			...
			if emoji == '💸':
					await msg.delete()
					embed_help = discord.Embed(description="📙┃Menu de Economia")
					...
					embed_help.add_field(name = 'Trabalhar',value ='Como usar ``s!trabalhar` - ',inline = False)																
					embed_help.add_field(name='-----------------------', value='***<arg> obrigatorio\n[arg] opicional***')				
					msg = await member.send(embed=embed_help)
					await msg.add_reaction('⬅')							
			if emoji == '⬅':
